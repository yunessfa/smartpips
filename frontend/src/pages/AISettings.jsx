...
const PROVIDERS = [
  ...
  ["github", "GitHub Models (free, token)", "https://models.github.ai/inference", "openai/gpt-4o-mini"],
...
export default function AISettings() {
  ...
  async function test(id) {
    ...
    try { const res = await aiApi.test(id); setTestResult((r) => ({ ...r, [id]: { ok: res.ok, msg: res.reply, rate: res.rate } })); }
                  ...
                  <li key={p.id} className="px-3 py-3 rounded-lg bg-ink-800 border border-ink-500">
                    ...
                    {tr && (
                      <p className={`text-xs mt-2 ${tr.loading ? "text-mist-500" : tr.ok ? "text-up" : "text-down"}`}>
                        {tr.loading ? t("testing") : tr.ok ? `✓ ${t("connected")}: ${tr.msg}${tr.rate ? ` · ${tr.rate}` : ""}` : `✗ ${tr.msg}`}
                      </p>
                    )}
                  </li>
                );
