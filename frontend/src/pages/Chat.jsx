...
export default function Chat() {
  ...
  async function send(text) {
    ...
    setSending(true);
    try {
      const res = await chatApi.send(content, activeId);
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="border-t border-ink-500 bg-ink-800/70 backdrop-blur px-4 md:px-8 py-4">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-[11px] text-mist-500 mt-2 text-center">{t("chat_disclaimer")}</p>
          </div>
        </div>
      </div>
    </div>
...
function Welcome({ onPick }) {
  ...
  return (
    <div className="text-center py-10">
      <span className="inline-grid place-items-center h-14 w-14 rounded-2xl bg-gradient-to-br from-gold to-gold-soft text-ink-900 font-bold text-2xl mb-4 shadow-lg shadow-gold/20">S</span>
...
function RecommendationCard({ rec, studies }) {
  ...
  const initial = {
    ...
    entry_price: toNum(rec.entry),
    take_profit: toNum(rec.take_profit),
    stop_loss: toNum(rec.stop_loss),
    source: "assistant",
  };
