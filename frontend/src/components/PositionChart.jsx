...
export function PositionChart({
  ...
  // Manual re-fit, since refreshes no longer do it behind your back.
  function resetZoom() {
  ...
  // The chart may only auto-fit when the VIEW itself changes (new symbol, new
  // timeframe, new chart instance). A background refresh must never touch the
  // time scale, otherwise every 15s poll threw away whatever the user had
  // zoomed into. `viewKeyRef` remembers which view the series currently holds
  // and `lastBarsRef` the bars already pushed, so a refresh can be applied as
  // an incremental update instead of a full reset.
  ...
  useEffect(() => {
    ...
    // A brand-new chart instance holds no data: force the candle effect below
    // to seed it and fit exactly once.
  ...
  // Feed candles WITHOUT stealing the user's view.
  //
  // First load of a symbol/timeframe: seed with setData + one fitContent.
  // Every later poll: push only the bars at or after the last known bar with
  // series.update(), which leaves zoom, scroll and crosshair untouched.
  useEffect(() => {
    ...
    if (prevLast === null) {
      ...
      // Bars are oldest -> newest, so this slice is already in ascending order,
      // which is what update() requires. It covers both "the live candle moved"
      // and "one or more new candles closed".
      const fresh = bars.filter((b) => b.time >= prevLast);
      if (fresh.length) fresh.forEach((b) => series.update(b));
      else series.setData(bars);   // history rewritten upstream: resync quietly
    }
    lastBarsRef.current = bars;
  }, [candles, symbol, tf, chartEpoch]);

  // Redraw the overlay only when a line actually moves. Positions are polled
  // every 5s and produce fresh objects each time; without this signature the
  // price lines were destroyed and recreated constantly, which made labels
  // flicker while you were trying to read the chart.
  const overlaySig = selected
    ? [selected.key, selected.entry, selected.tp, selected.sl, selected.liq,
       selected.mark].join("|")
    : "";
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={resetZoom} title="Reset zoom"
            className="px-2 py-1 text-[11px] rounded-md border border-ink-500 bg-ink-800
              text-mist-400 hover:text-mist-100 hover:border-ink-400 transition">
            ⤢
          </button>
