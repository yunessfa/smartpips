...
def get_quotes(assets) -> list[dict]:
    ...
    for asset in assets:
        ...
        if asset.binance_symbol:
            ...
            price = fetch_metal_price(asset.symbol)
            if price:
                quote = {
                    "price": round(price, 2),
                    "change_percent": 0.0,  # spot endpoint has no 24h change
                    "high": None, "low": None, "volume": None,
                    "source": "metals",
                }
        elif asset.category == "forex":
            from .quotes_resolver import live_price
            price = live_price(asset.symbol)
            if price:
                quote = {
                    "price": round(price, 5),
                    "change_percent": 0.0,
